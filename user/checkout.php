    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div class="rounded-xl border bg-white p-5 shadow-sm">
            <div class="mb-4 flex items-center gap-3">
                <div
                    class="flex h-11 w-11 items-center justify-center rounded bg-fuchsia-700 text-sm font-bold uppercase tracking-wide text-white">
                    MoMo
                </div>
                <div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between gap-3 border-b pb-3 last:border-b-0 last:pb-0">
                    <div>
                        ...
                        <div class="text-sm text-gray-500">Size <?= (int)$it['shoe_size'] ?> • SL
                            <?= (int)$it['quantity'] ?></div>
                    </div>
            ...
            <div class="mt-5 flex items-center justify-between border-t pt-4">
                ...
                <span class="text-2xl font-bold text-fuchsia-700"><?= number_format((float)$total, 0, ',', '.') ?>
                    VND</span>
            </div>
            ...
            <form method="POST" action="<?= e(app_url('user/checkout_confirm.php')) ?>" class="mt-5 space-y-3">
                <div class="rounded-lg border bg-gray-50 p-4">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <div>
                            ...
                            <input name="buyer_phone" value="<?= e($old_phone) ?>" required inputmode="tel"
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            <div>
                                ...
                                <input name="addr_house" value="<?= e($old_house) ?>" required
                                    class="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm"
                                    placeholder="VD: 12A">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700">Thôn</label>
                                <input name="addr_hamlet" value="<?= e($old_hamlet) ?>" required
                                    class="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm"
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            <div>
                                ...
                                <input name="addr_commune" value="<?= e($old_commune) ?>" required
                                    class="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm"
                                    placeholder="VD: Xã ABC">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700">Tỉnh</label>
                                <input name="addr_province" value="<?= e($old_province) ?>" required
                                    class="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm"
                    ...
                    <div class="space-y-2 text-sm text-gray-700">
                        ...
                        <label class="flex cursor-pointer items-start gap-3 rounded border bg-white p-3">
                            ...
                            <span>
                                ...
                                <span class="block text-gray-500">Tạo đơn ngay, trạng thái thanh toán sẽ để chờ xử
                                    lý.</span>
                            </span>
                        </label>
                    </div>
                ...
                <button type="submit"
                    class="w-full rounded-lg bg-fuchsia-700 px-4 py-3 font-medium text-white hover:bg-fuchsia-800">
                    Tiếp tục thanh toán
                </button>
            </form>

            <div class="mt-3 text-center">
                <a href="<?= e(app_url('user/cart.php')) ?>" class="text-sm text-fuchsia-700 hover:underline">Quay lại
                    giỏ hàng</a>
            </div>
        </div>
      ...
      <p class="mt-4 text-sm text-fuchsia-800">
        Nếu MoMo sandbox không gọi được `IPN URL` từ internet, đơn vẫn có thể được cập nhật khi người dùng quay
        lại `returnUrl`.
      </p>
    </div> -->
    </div>
